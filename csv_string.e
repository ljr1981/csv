...
create
	...
	make_from_escaped_csv_string,
	make_with_escaped_csv, make_csv, make_csv_from_string_32
...
feature {NONE} -- Initialization
	...
	make_from_string (s: READABLE_STRING_8)
		...
		require
			s_not_void: s /= Void
		do
			make_from_escaped_csv_string (escaped_csv_string (s))
		end
	...
	make_from_escaped_csv_string (a_escaped_string: READABLE_STRING_8)
			-- Initialize with `a_escaped_string' already csv escaped.
		require
	...
	make_with_escaped_csv (a_escaped_string: READABLE_STRING_8)
			-- Initialize with `a_escaped_string' already csv escaped.
		obsolete
			"Use `make_from_escaped_csv_string' [2017-05-31]"
		require
			a_escaped_string_not_void: a_escaped_string /= Void
		do
			make_from_escaped_csv_string (a_escaped_string)
		end

	make_from_csv_string (a_csv: csv_STRING)
			-- Initialize with `a_csv' string value.
		do
			make_from_escaped_csv_string (a_csv.item)
		end

	make_csv (s: READABLE_STRING_8)
			-- Initialize.
		obsolete
		...
		do
			make_with_escaped_csv (escaped_csv_string (s))
		end

	make_csv_from_string_32 (s: READABLE_STRING_32)
...
feature -- Conversion
	...
	unescaped_string_32: STRING_32
			...
			--| some encoders uses UTF-8 , and not the recommended pure csv encoding
...
feature -- Comparison
	...
	is_equal (other: like Current): BOOLEAN
			-- Is csv_STRING  made of same character sequence as `other'
...
feature -- Change Element
	...
	append (a_escaped_string: READABLE_STRING_8)
			-- Add csv escaped string `a_escaped_string'
		require
			a_escaped_string_not_void: a_escaped_string /= Void
		do
	...
	append_csv_string (a_csv_string: csv_STRING)
			-- Add csv string `a_csv_string'
		require
			a_csv_string_not_void: a_csv_string /= Void
		do
			append (a_csv_string.item)
		end
...
feature {NONE} -- Implementation
	...
	escaped_csv_string (s: READABLE_STRING_GENERAL): STRING_8
			-- csv string with '"' and '\' characters and unicode escaped
		require
			s_not_void: s /= Void
		local
			i, j, n: INTEGER
